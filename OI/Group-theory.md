...
$\color{red}\text{定理3.3:}$ 称 $E_{1\sim n}$ 中本质不同的个数为等价类个数，设为 $L$。
...
设旋转 $0\degree, 90\degree, 180\degree, 270\degree$ 为四种置换，将 $16$ 种染色方法看做元素。则 $G$ 是所有染色方法的一个置换群，即 $|G|=4$。

- 那么旋转 $0\degree$ 为不动置换。即 $c(p_1)=16$，有 $16$ 个不动点。
- 旋转 $90\degree$：手算一下有 $c(p_2)=2$。
- 旋转 $180\degree$: 再手算一下有 $c(p_3)=4$。
- 旋转 $270\degree$: 手算一下 $c(p_4)=2$。
...
$\color{red}{定理3.4:}$ 用 $m$ 种颜色，涂染 $n$ 个对象，则不同的染色方案为：
