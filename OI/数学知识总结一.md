...
充分性：（所有合数均不满足上式）显然，考虑分解 $p=a*b$ 即可得到 $(p-1)!\equiv 0\pmod p$。

必要性：

原命题等价于证：$2 * 3 * \dots (p - 2)\equiv 1\pmod p$

引理：$\forall i\in [2, p-2], inv(i)\in [2, p-2]$ ， $inv(i)$ 唯一且不重复。

用人话说就是：$2\sim p-2$ 中的数两两配对，且不会重复。配对的两个数满足乘积在模意义下为 $1$ 。
...
例题：
给出素数 $p$，求 $q!\bmod p$，其中 $q$ 为比 $p$ 小的最大素数，$p \in [1, 10^9]$。

解：
素数的分布较为密集，直接从 $p$ 暴力找出 $q$ 即可。

观察到模数限制，考虑构造 $p!$ 来使用威尔逊定理。

具体的，$q!=\frac{p!}{p(p-1)(p-2)\dots (q + 1)}$
...
引理：对于满足 $K^x\equiv 1\pmod p$ 的最小 $x$ ，必然有 $x\mid \varphi(p)$。
...
$(x+y)^n = \sum_{k = 0}^{n}{\binom{n}{k}\times x^k \times y^{n - k}}$
...
- 组合恒等式（尝试证明）
1. $\frac{i}{n}{\binom{n}{i}} = \binom{n-1}{i-1}$：

这条性质很有用，可以用它来吸收或者释放组合数前讨厌的系数。
证明：展开上式即可。

2. $\binom{n+1}{m}=\binom{n}{m}+\binom{n}{m-1}$

证明：展开即可。

3. $\binom{m+r+1}{r}=\sum_{i=0}^r \binom{m+i}{i}$
...
- $\binom{m}{0}+\binom{m}{2}+\binom{m}{4}...=\binom{m}{1}+\binom{m}{3}+\binom{m}{5}+...=2^{m-1}$

考虑 $(1-1)^m$ 的二项式展开。

- $\sum_{i=1}^n \binom{n}{i}*i^2=n*(n+1)*2^{n-2}$
